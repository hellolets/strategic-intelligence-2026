#!/usr/bin/env python3
"""
Script para aplicar la integración del nuevo consolidator en processor.py
"""

import re

# Leer el archivo
with open('deep_research/processor.py', 'r', encoding='utf-8') as f:
    content = f.read()

# Buscar la sección a reemplazar
old_pattern = r'print\(f"      🤖 Generando documento completo con \{CURRENT_CONSOLIDATOR_MODEL\}\.\.\."\)\s+print\(f"      ⏳ Esto puede tardar varios minutos debido al tamaño del contenido\.\.\."\)\s+try:\s+response = llm_consolidator\.invoke\([^)]+\)\s+final_report = response\.content if hasattr\(response, "content"\) else str\(response\)[^}]+except Exception as e:[^}]+final_report \+= consolidated_references_section'
...
print("⚠️  Este script requiere edición manual del archivo processor.py")
print("   Busca la sección que comienza con:")
print('   print(f"      🤖 Generando documento completo con {CURRENT_CONSOLIDATOR_MODEL}...")')
print("   Y reemplázala con el nuevo código del pipeline por etapas.")
